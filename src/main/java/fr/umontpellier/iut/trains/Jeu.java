...
public class Jeu implements Runnable {
    ...
    public void run() {
        ...
        log("<div class=\"tour\">Initialisation</div>");
        // boucle principale
        while (!estFini()) {
            log("<div class=\"tour\">Tour de " + joueurCourant.toLog() + "</div>");
    ...
    public boolean estFini() {
        ...
        for (ListeDeCartes pile : reserve.values()) {
            if (pile.isEmpty() && !pile.getNom().equals("Ferraille")) {
                nbPilesVides++;
            }
        }
        return nbPilesVides >= 4 || joueurCourant.getNbJetonsRail() == 0 || nbJetonsGare == 0;
    }
